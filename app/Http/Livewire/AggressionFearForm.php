...
class AggressionFearForm extends Component
{
            ...
            foreach ($behaviors as $behavior) {
                ...
                if ($behavior['behavior_id'] == 28) {
                    if (strlen($behavior['value']) > 0) {
                        $this->state['question22']['value'] = explode(',', $behavior['value']);
                    }
                    if (strlen($behavior['comments']) > 0) {
                        $this->state['question22']['comments'] = $behavior['comments'];
                    }
                }
                //Fill multiple pption question: question23 - #7 on view
                else if ($behavior['behavior_id'] == 29) {
                    if (strlen($behavior['value']) > 0) {
                        $this->state['question23']['value'] = explode(',', $behavior['value']);
                    }
                    if (strlen($behavior['comments']) > 0) {
                        $this->state['question23']['comments'] = $behavior['comments'];
                    }
                } else {
    ...
    public function save()
    {
        ...
        //Validate
        $validate = Validator::make($this->state, $this->rules(), [], $this->customAttributes())->validateWithBag('save');
        ...
        if ((count($this->state['question22']['value']) > 0) || (strlen($this->state['question22']['comments']) > 0)) {
            $answers[28] = [
        ...
        if ((count($this->state['question23']['value']) > 0) || (strlen($this->state['question23']['comments']) > 0)) {
            $answers[29] = [
